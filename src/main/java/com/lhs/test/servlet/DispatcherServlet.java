...
import com.lhs.test.utils.ClassTools;


/**
 * 1.加载指定目录下的所有class文件
   2.将使用@controller和@service注解的类放到map,key就是类名小写或者自己定义的，value是被注解类的对象
   3.对于map中的值，查看它的属性是否被@autowired注解，如果是，为这个对象的属性，赋值
   4.查看map中对象是否被@requestparam注入了，是，则取出注解的值拼装成url,然后将url和map中的对象及其方法做mapping
 * @author LiHuaSheng
 *
 */
public class DispatcherServlet extends HttpServlet {
	...
	/** 要扫描的包，只有在这个包下并且加了注解的才会被扫描到 */
	private static final String PACKAGE = "com.lhs.test";
    ...
    public void init(ServletConfig config) throws ServletException { 
        ...
        // 开始扫描包下全部class文件
        List<String> classes = getClasses(PACKAGE);
	 ...
	 * @return
	 */
	private List<Object> buildParamObject(HttpServletRequest req,
			HttpServletResponse resp, Method method) {
		...
		for (Parameter parameter : parameters) {
			// 当前参数有别名注解并且别名不为空
			if (parameter.getParameterizedType().getTypeName()
					.contains("HttpServletRequest")) {
				paramValue.add(req);
			} else if (parameter.getParameterizedType().getTypeName()
					.contains("HttpServletResponse")) {
				paramValue.add(resp);
			}else if(parameter.isAnnotationPresent(RequestParam.class)
					&& !parameter.getAnnotation(RequestParam.class).value()
							.isEmpty()) {
				// 我们获取
				String value = req.getParameter(parameter.getAnnotation(
	...
	private void buildUrlMethodMapping(Map<String, Object> instanceMap,
		...
		for (Entry<String, Object> entry : instanceMap.entrySet()) {
			...
			if (entry.getValue().getClass()
				...
				parenturl = entry.getValue().getClass()
	 ...
	 * @param instanceMap
	 */
	private void doIoc(Map<String, Object> instanceMap,
			Map<Class<?>, Object> instanceTypeMap) {
			...
			for (Field field : fields) {
				...
				if (field.isAnnotationPresent(Autowired.class)) {
					...
					if (field.getAnnotation(Autowired.class).value().isEmpty()) {
						...
						try {
							// 将被注入的对象
							Object object = instanceMap.get(field
	 ...
	 */
	private void buildInstanceMap(List<String> classes,
		...
		for (String str : classes) {
			Class<?> clasz = this.getClass().getClassLoader().loadClass(str);
			// 组装instanceMap
			...
			if (clasz.isAnnotationPresent(Controller.class)) {
				try {
					...
					throw new RuntimeException(
							"初始化instanceMap时在处理Controller注解时出现了异常");
				}
				...
				try {
					...
					throw new RuntimeException(
							"初始化instanceMap时在处理Service注解时出现了异常");
				}
				// 实现的接口数组
	...
	public List<String> getClasses(String pack) {
